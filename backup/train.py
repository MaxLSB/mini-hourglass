...
model = LanguageModel(n_blocks=n_blocks, vocab_size=vocab_size, n_heads=n_heads,
                      n_embedding=n_embedding, block_size=block_size, device=device).to(device)

optimizer = torch.optim.AdamW(model.parameters(), lr=learning_rate)
loss_fn = torch.nn.CrossEntropyLoss()

print(f'Parameters: {sum(parameter.numel()
      for parameter in model.parameters()) / 1e6:.2f}M')
print(f'Vocab_size: {vocab_size}, Block_size: {block_size}, Batch_size: {
      batch_size}, N_blocks: {n_blocks}, N_heads: {n_heads}, N_embedding: {n_embedding}')
...
for epoch in range(epochs):
    ...
    with torch.no_grad():
        ...
        for x, y in test_loader:
            x, y = x.to(device), y.to(device)
            y_pred = model(x)
            total_loss += loss_fn(y_pred.view(-1, vocab_size),
                                  y.view(-1)).item()
        print(f'Epoch: {epoch}, Training Loss: {loss.item()
                                                }, Val loss: {total_loss / len(test_loader)}')
