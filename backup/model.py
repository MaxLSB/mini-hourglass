...
class MultiHeadAttention(torch.nn.Module):
    def __init__(self, n_heads, head_size, n_embedding, block_size):
        ...
        self.heads = torch.nn.ModuleList([AttentionHead(
            self.head_size, self.n_embedding, self.block_size) for _ in range(n_heads)])
...
class Block(torch.nn.Module):
    def __init__(self, n_heads, n_embedding, block_size):
        ...
        self.attn = MultiHeadAttention(
            self.n_heads, self.head_size, self.n_embedding, self.block_size)
        self.ffn = FeedForward(self.n_embedding)

    def forward(self, x):
        # In GPT like models we use Pre-LayerNorm instead of Post-LayerNorm
...
class LanguageModel(torch.nn.Module):
    def __init__(self, n_blocks, vocab_size, n_heads, n_embedding, block_size, device):
        ...
        self.position_embedding = PositionalEncoding(
            self.block_size, self.n_embedding, self.device)
        self.tokens_embedding = torch.nn.Embedding(
            self.vocab_size, self.n_embedding)

        self.blocks = torch.nn.ModuleList([Block(
            self.n_heads, self.n_embedding, self.block_size) for _ in range(self.n_blocks)])
